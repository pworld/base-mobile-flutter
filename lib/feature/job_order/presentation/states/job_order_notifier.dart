...
class JobOrderNewStateNotifier extends StateNotifier<JobOrdersListState> {
  ...
  JobOrderNewStateNotifier(this._jobOrderNewUseCase)
      : super(JobOrdersListState.initial());
  ...
  Future<void> jobOrderNew(RequestPage requestData) async {
    ...
    response.fold(
      (failure) {
        ...
        state = JobOrdersListState(
            successData: AsyncValue.error(failure, StackTrace.current));
      },
      (success) {
...
class transferVehicleStateNotifier extends StateNotifier<TransferVehicleState> {
  ...
  transferVehicleStateNotifier(this._jobOrderNewUseCase)
      : super(TransferVehicleState.initial());
  ...
  Future<void> fetchJobOrderTransferVehicle(String jobOrderId) async {
    final response =
        await _jobOrderNewUseCase.fetchJobOrderTransferVehicle(jobOrderId);
    response.fold(
      (failure) {
        // Handle the failure case, providing both the failure and the current stack trace
        state = TransferVehicleState(
            successData: AsyncValue.error(failure, StackTrace.current));
      },
      (success) {
        // Handle the success case
...
class updateTransferVehicleStateNotifier
    extends StateNotifier<TransferVehicleTripState> {
  final JobOrderNewUseCase _jobOrderNewUseCase;

  updateTransferVehicleStateNotifier(this._jobOrderNewUseCase)
      : super(TransferVehicleTripState.initial());

  void startLoading() {
    state = TransferVehicleTripState(successData: const AsyncValue.loading());
  }

  Future<TransferVehicleTripState> updateTransferVehicle(
      RequestTransferVehicleTripModel payload) async {
    final response =
        await _jobOrderNewUseCase.jobOrderTransferVehicleStartTrip(payload);
    response.fold(
      (failure) {
        // Handle the failure case, providing both the failure and the current stack trace
        state = TransferVehicleTripState(
            successData: AsyncValue.error(failure, StackTrace.current));
      },
      (success) {
        // Handle the success case
  ...
  Future<TransferVehicleTripState> startTransferVehicle(
      RequestTransferVehicleTripModel payload) async {
    final response =
        await _jobOrderNewUseCase.jobOrderTransferVehicleStartTrip(payload);
    response.fold(
  ...
  Future<TransferVehicleTripState> finishTransferVehicle(
      RequestTransferVehicleTripModel payload) async {
    final response =
        await _jobOrderNewUseCase.jobOrderTransferVehicleFinishTrip(payload);
    response.fold(
      (failure) {
        // Handle the failure case, providing both the failure and the current stack trace
        state = TransferVehicleTripState(
